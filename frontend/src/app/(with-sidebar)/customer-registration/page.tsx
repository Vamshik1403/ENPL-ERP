...
export default function CustomerContactPage() {
  ...
  const fetchPermissions = async (uid: number) => {
    try {
      if (localStorage.getItem('userType') === 'SUPERADMIN') { 
        ...
        setPerm(all); setLoadingPerms(false); return; 
      }
      const r = await fetch(`${PERMISSIONS_API}/${uid}`); 
      if (!r.ok) throw new Error();
      const raw = await r.text(); 
      if (!raw) { setLoadingPerms(false); return; }
      ...
      setPerm({ read: false, create: false, edit: false, delete: false }); 
    } finally { 
  ...
  const filtered = useMemo(() => {
    ...
    return records.filter(r => 
      r.custFirstName.toLowerCase().includes(t) || 
      r.custLastName.toLowerCase().includes(t) || 
      r.phoneNumber.includes(searchTerm) || 
      r.emailAddress.toLowerCase().includes(t) || 
      r.sites.some(s => s.site.siteName.toLowerCase().includes(t))
    );
  ...
  const paginated = useMemo(() => { 
    ...
    return filtered.slice(s, s + recordsPerPage); 
  ...
  const resetPanel = () => { 
    setShowPanel(false); 
    setEditingId(null); 
    setForm({ custFirstName: '', custLastName: '', phoneNumber: '', emailAddress: '' }); 
    setRows([]); 
  ...
  const addRow = () => {
    if (!draftCustomerId || !draftSiteId) { 
      toast({ title: 'Select customer and site', variant: 'warning' }); 
      return; 
    }
    ...
    // SAFE: Check if sites is array before using find
    const site = Array.isArray(sites) 
      ? sites.find(s => s.id === draftSiteId) 
      : undefined;
    
    if (!cust || !site) return;
    
    if (rows.some(r => r.customerId === cust.id && r.siteId === site.id)) { 
      toast({ title: 'Already added', variant: 'warning' }); 
      return; 
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!rows.length) { 
      toast({ title: 'Add at least one Customer + Site', variant: 'warning' }); 
      return; 
    }
    if (editingId && !perm.edit) { 
      toast({ title: 'No edit permission', variant: 'error' }); 
      return; 
    }
    if (!editingId && !perm.create) { 
    ...
    const payload = { 
      ...form, 
      sites: rows.map(r => ({ customerId: r.customerId, siteId: r.siteId })) 
    };
  ...
  if (!perm.read) return (
    <div className="flex items-center justify-center min-h-[60vh]">
      <div className="text-center p-8 bg-white rounded-xl border border-gray-200 shadow-sm max-w-md">
        ...
        <p className="text-sm text-gray-500">You don&apos;t have permission to view customer contacts.</p>
      </div>
    </div>
  );
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Manage customer contacts and site assignments</p>
        </div>
        <Button onClick={() => setShowPanel(true)} disabled={!perm.create} className="gap-2">
                ...
                <TableRow key={r.id} className="hover:bg-gray-50/50 text-sm">
                  <TableCell>
                    <div className="flex items-center gap-3">
                      <div className="h-8 w-8 rounded-full bg-indigo-100 flex items-center justify-center text-xs font-semibold text-indigo-600">
                  ...
                  <TableCell>
                    <div className="flex flex-wrap gap-1">
                      {r.sites.map(s => <Badge key={s.id} variant="outline" className="text-xs">{s.site.siteName}</Badge>)}
                    </div>
                  </TableCell>
      ...
      <SlideFormPanel 
        ...
        description="Manage contact details and site assignments" 
        isOpen={showPanel} 
        onClose={resetPanel} 
      >
        <form onSubmit={handleSubmit} className="p-6 space-y-6 overflow-y-auto max-h-[calc(100vh-120px)]">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
              ...
              <div className="md:col-span-2">
                <Button type="button" onClick={addRow} variant="outline" className="w-full gap-1">
            ...
            {rows.length > 0 && (
              <div className="border border-gray-200 rounded-lg overflow-hidden">
                <div className="bg-gray-50 px-4 py-2 border-b">
                  <p className="text-xs font-medium text-gray-700">Assigned Sites ({rows.length})</p>
                </div>
                <Table>
                  <TableHeader>
                    <TableRow className="bg-gray-50/80">
          ...
          <div className="flex items-center gap-3">
            <Button 
              type="submit" 
              disabled={editingId ? !perm.edit : !perm.create} 
              className="flex-1"
            >
              {editingId ? 'Update Contact' : 'Create Contact'}
            </Button>
            <Button type="button" variant="outline" onClick={resetPanel} className="flex-1">
              Cancel
            </Button>
          </div>
        </form>
