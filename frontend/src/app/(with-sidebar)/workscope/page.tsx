...
export default function WorkscopeCategoryPage() {
  const [categories, setCategories] = useState<WorkscopeCategory[]>([]);
  ...
  const fetchPermissions = async (uid: number) => {
    try {
      ...
      if (storedUserType === 'SUPERADMIN') {
        ...
        setAllPermissions({ WORKSCOPE_CATEGORY: all });
        setPerms(all);
      ...
      setAllPermissions(p);
      setPerms(p.WORKSCOPE_CATEGORY ?? { read: false, create: false, edit: false, delete: false });
  ...
  const filtered = categories.filter(c =>
    c.workscopeCategoryName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    try {
      if (editingId) {
        const res = await fetch(`${API_URL}/${editingId}`, {
          ...
          body: JSON.stringify({ workscopeCategoryName: formData.workscopeCategoryName }),
        });
        if (!res.ok) throw new Error('Failed');
      } else {
        const res = await fetch(API_URL, {
  ...
  const handleEdit = (id: number) => {
    ...
    if (item) {
      setFormData({ workscopeCategoryName: item.workscopeCategoryName });
      setEditingId(id);
      setShowPanel(true);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    if (!confirm('Are you sure you want to delete this workscope category?')) return;
    try {
  ...
  const resetForm = () => { setShowPanel(false); setEditingId(null); setFormData({ workscopeCategoryName: '' }); };
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center p-8 bg-white rounded-xl border border-gray-200 shadow-sm max-w-md">
          ...
          <p className="text-sm text-gray-500">You don&apos;t have permission to view workscope categories.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-xl font-semibold text-gray-900">Workscope Categories</h1>
          <p className="text-sm text-gray-500 mt-0.5">Manage workscope category definitions</p>
        </div>
            ...
            {loading ? (
              ...
              currentItems.map(item => (
                <TableRow key={item.id} className="hover:bg-gray-50/50">
                  ...
                  <TableCell className="font-medium text-gray-900">{item.workscopeCategoryName}</TableCell>
      ...
      <SlideFormPanel title={editingId ? 'Edit Category' : 'Add Category'} description={editingId ? 'Update the workscope category' : 'Create a new workscope category'} isOpen={showPanel} onClose={resetForm} >
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          <div className="space-y-2">
            <Label htmlFor="categoryName">Category Name</Label>
            <Input id="categoryName" value={formData.workscopeCategoryName} onChange={e => setFormData({ ...formData, workscopeCategoryName: e.target.value })} placeholder="Enter category name" required autoFocus />
          </div>
          <Separator />
